...
export class BST<T> {
  ...
  public invert() {
    this.root = this.invertHelper(this.root);
  }

  private invertHelper(root?: BSTNode<T> | undefined): BSTNode<T> | undefined {
    if (root === undefined) return root;

    const left = this.invertHelper(root.getRight());
    const right = this.invertHelper(root.getLeft());
